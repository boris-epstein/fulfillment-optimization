    ...
    for supply_node_id in supply_nodes:
        if supply_node_id == 0:
            for demand_node_id in demand_nodes:
                if demand_node_id ==0:
                    rewards[supply_node_id,demand_node_id] = 1
                else:
                    rewards[supply_node_id,demand_node_id] =  np.random.uniform()
        
        else:
            rewards[supply_node_id, supply_node_id] = 1
...
def save_nodes(nodes, type = 'supply', dir = ''):
    path = dir + f'starlike_{type}nodes_seed={RANDOM_SEED}.csv'
...
def save_rewards(rewards, dir = ''):
    path = dir + f'starlike_rewards_seed={RANDOM_SEED}.csv'
    with open(path, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile, delimiter = ',')

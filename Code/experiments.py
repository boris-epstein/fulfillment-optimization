...
from FulfillmentOptimization import (
    Fulfillment, Inventory, PolicyFulfillment, MultiPriceBalanceFulfillment,
    BalanceFulfillment, FluLpReSolvingFulfillment, OffLpReSolvingFulfillment,
...
from Demand import (
    TemporalIndependenceGenerator, RWGenerator, MarkovianGenerator,
    Sequence, RandomDistributionGenerator, HiddenMarkovGenerator
)
from ModelFree import (
    ThresholdsFulfillment, TimeSupplyEnhancedMPB, TimeEnhancedMPB,
    SupplyEnhancedMPB, NeuralOpportunityCostPolicy, NeuralOpportunityCostWithIDPolicy
)
...
def generate_experiment_logging_setup(demand_model: str, base_log_dir: str = "logs"):
    """Create a unique experiment directory and log file path.

    Args:
        demand_model: Name of the demand model (used in the experiment ID).
        base_log_dir: Root directory for experiment logs.

    Returns:
        Tuple of (experiment_id, experiment_dir, master_log_path).
    """
...
def run_single_instance(args):
    """Run all policies on a single problem instance (used as a multiprocessing target).

    Args:
        args: Tuple containing instance_id, instance, demand_model, training config,
            policy lists, and experiment directory.

    Returns:
        Tuple of (instance_id, results_dict).
    """
    (
        instance_id,
        instance,
    ...
    def setup_instance_logger(experiment_dir: str, instance_id: int):
        """Create a file logger for a specific instance."""
...
def generate_experiment_id(demand_model: str) -> str:
    """Generate a unique experiment ID from the demand model name and current time."""
    now = datetime.now().isoformat()
    hash_id = hashlib.md5(now.encode()).hexdigest()[:8]
...
def setup_logging(experiment_id):
    """Configure file and console logging for an experiment."""
    log_dir = "logs"
...
class Instance:
    """A problem instance consisting of a graph and its demand distribution.

    Attributes:
        graph: The bipartite supply-demand graph.
        distribution: The demand distribution parameters.
        distribution_type: Type of demand model ('indep', 'markov', 'rw', 'correl').
    """
...
class PolicyOutput:
    """Stores the performance results of a specific policy on one instance.

    Attributes:
        rewards: Average test reward(s). Scalar for data-agnostic policies,
            list for data-driven policies (one per training sample).
        train_times: Training time(s).
        test_times: Average testing time(s).
        train_set_size: Number of training samples used.
        policy_name: Name of the policy.
    """

    def __init__(self, rewards: List[float], train_times, test_times,
                 train_set_size: int, policy_name: str = ''):
...
def log_output_line(instance_id, policy_name, sample_size, sample_id,
                    average_test_reward, training_time, average_testing_time):
    """Log a single result line in CSV format."""
...
class OutputWriter:
    ...
    Handles both data-agnostic policies (single row per instance) and
    data-driven policies (one row per instance x sample_size x sample_id).
    """
    ...
    def write_output(self, path):
        """Write all results to a CSV file at the given path."""
        with open(path, 'w', newline='') as csv_file:
            spamwriter = csv.writer(csv_file, delimiter=',')
            spamwriter.writerow(['instance_id', 'policy_name', 'sample_size', 'sample_id',
                                 'average_test_reward', 'training_time', 'average_testing_time'])

            for instance_id in range(self.num_instances):
                if self.include_optimal:
                    r = self.results[instance_id]['optimal']
                    spamwriter.writerow([instance_id, 'optimal', 0, 0, r.rewards, r.train_times, r.test_times])

                for policy in self.data_agnostic_policies:
                    r = self.results[instance_id][policy]
                    spamwriter.writerow([instance_id, policy, 0, 0, r.rewards, r.train_times, r.test_times])
                ...
                for policy in self.model_based_dynamic_programs + self.model_free_parametrized_policies + self.lp_resolving_policies:
                    for num_samples in self.train_sample_sizes:
                        for sample_id in range(self.num_samples_per_size):
                            r = self.results[instance_id][policy][num_samples]
                            spamwriter.writerow([
                                instance_id, policy, num_samples, sample_id,
                                r.rewards[sample_id], r.train_times[sample_id], r.test_times[sample_id]
                            ])


def compute_cumulative_average_demand(train_sample: List[Sequence], graph: Graph):
    """Compute backward-cumulative average demand from training samples.

    For each time step t, computes the average number of requests for each
    demand node from t onward (inclusive) across all training sequences.

    Args:
        train_sample: List of training Sequence objects.
        graph: The bipartite graph (for demand node IDs).

    Returns:
        Dict mapping time step t to a dict of {demand_node_id: cumulative_count}.
    """
...
class Experiment:
    """Runs all configured policies on a single graph instance.

    Handles training (where applicable), testing, timing, and result collection
    for data-agnostic, model-based, model-free, and LP re-solving policies.
    """
    ...
    def conduct_experiment(self) -> Dict[any, PolicyOutput]:
        ...
        Returns:
            Dict mapping policy name (or policy name -> sample size for data-driven)
            to PolicyOutput.
        """
        instance_results = {}
    ...
    def process_offline(self):
        """Evaluate the offline LP upper bound on the test set."""
        start = time.time()
    ...
    def process_optimal(self):
        """Evaluate the optimal DP policy on the test set."""
    ...
    def process_myopic(self):
        """Evaluate the myopic (greedy by reward) policy on the test set."""
        start = time.time()
        myopic_scores = {(edge.supply_node_id, edge.demand_node_id): edge.reward for edge in self.graph.edges.values()}
        self.graph.construct_priority_list('myopic', myopic_scores, allow_rejections=False)
    ...
    def process_balance(self):
        """Evaluate the multi-price balance policy on the test set."""
        start = time.time()
    ...
    def process_model_free(self, policy: str):
        """Train and evaluate a model-free parametrized policy.

        Trains using Nevergrad optimization on each training sample, then
        evaluates the best parameters on the test set.

        Returns:
            Dict mapping sample_size to PolicyOutput.
        """
        ...
        for n_train_samples in self.train_sample_sizes:
            for i in range(self.n_samples_per_size):
                ...
                log_output_line(self.instance_id, policy, n_train_samples, i,
                                rewards[n_train_samples][i], train_times[n_train_samples][i],
                                test_times[n_train_samples][i])

        policy_output = {}
        for n_train_samples in self.train_sample_sizes:
            policy_output[n_train_samples] = PolicyOutput(
                rewards[n_train_samples], train_times[n_train_samples],
                test_times[n_train_samples], n_train_samples, policy
            )
    ...
    def process_dp(self, dp_name):
        """Train and evaluate a model-based DP policy.

        Estimates the demand model from training data, computes the optimal DP
        policy, and evaluates on test sequences.
        ...
        for n_train_samples in self.train_sample_sizes:
            for i in range(self.n_samples_per_size):
                ...
                log_output_line(self.instance_id, dp_name, n_train_samples, i,
                                rewards[n_train_samples][i], train_times[n_train_samples][i],
                                test_times[n_train_samples][i])
        ...
        for n_train_samples in self.train_sample_sizes:
            policy_output[n_train_samples] = PolicyOutput(
                rewards[n_train_samples], train_times[n_train_samples],
                test_times[n_train_samples], n_train_samples, dp_name
            )
    ...
    def process_lp(self, lp_name):
        """Dispatch to the appropriate LP re-solving method."""
    ...
    def _get_re_solving_epochs(self):
        """Return equi-spaced re-solving epochs based on the time horizon T."""
        if self.T == 6:
            return [1, 2, 3, 4, 5]
        if self.T == 12:
            return [2, 4, 6, 8, 10]
    ...
    def process_fluid_lp_resolving(self):
        """Train and evaluate the fluid LP re-solving policy.

        Returns:
            Dict mapping sample_size to PolicyOutput.
        """
        re_solving_epochs = self._get_re_solving_epochs()
        ...
        for n_train_samples in self.train_sample_sizes:
            for sample_id in range(self.n_samples_per_size):
                ...
                backwards_cumulative_average_demand[n_train_samples].append(
                    compute_cumulative_average_demand(train_sample, self.graph)
                )

                initial_dual_variables[n_train_samples].append(
                    fulfiller.compute_dual_variables(
                        0, self.inventory.initial_inventory,
                        backwards_cumulative_average_demand[n_train_samples][sample_id]
                    )
                )
            ...
            for i in range(self.n_samples_per_size):
                ...
                for sequence in self.test_samples:
                    _, reward, _ = fulfiller.fulfill(
                        sequence, self.inventory,
                        initial_dual_variables[n_train_samples][i],
                        backwards_cumulative_average_demand[n_train_samples][i],
                        re_solving_epochs
                    )
                    rewards[n_train_samples][i] += reward / self.n_test_samples
                test_times[n_train_samples].append((time.time() - start) / self.n_test_samples)

                log_output_line(self.instance_id, 'fluid_lp_resolving', n_train_samples, i,
                                rewards[n_train_samples][i], train_times[n_train_samples][i],
                                test_times[n_train_samples][i])
        ...
        for n_train_samples in self.train_sample_sizes:
            policy_output[n_train_samples] = PolicyOutput(
                rewards[n_train_samples], train_times[n_train_samples],
                test_times[n_train_samples], n_train_samples, 'fluid_lp_resolving'
            )

        return policy_output

    def process_off_lp_resolving(self):
        """Train and evaluate the offline LP re-solving policy.
        ...
        for n_train_samples in self.train_sample_sizes:
            for sample_id in range(self.n_samples_per_size):
                ...
                initial_dual_variables[n_train_samples].append(
                    fulfiller.compute_dual_variables(train_sample, 0, self.inventory.initial_inventory, None, False)
                )
            ...
            for i in range(self.n_samples_per_size):
                ...
                for sequence in self.test_samples:
                    _, reward, _ = fulfiller.fulfill(
                        sequence, self.inventory,
                        initial_dual_variables[n_train_samples][i],
                        self.train_samples[n_train_samples][i],
                        re_solving_epochs
                    )
                    rewards[n_train_samples][i] += reward / self.n_test_samples
                test_times[n_train_samples].append((time.time() - start) / self.n_test_samples)

                log_output_line(self.instance_id, 'offline_lp_resolving', n_train_samples, i,
                                rewards[n_train_samples][i], train_times[n_train_samples][i],
                                test_times[n_train_samples][i])
        ...
        for n_train_samples in self.train_sample_sizes:
            policy_output[n_train_samples] = PolicyOutput(
                rewards[n_train_samples], train_times[n_train_samples],
                test_times[n_train_samples], n_train_samples, 'offline_lp_resolving'
            )

        return policy_output

    def process_fluid_extrapolation(self):
        """Train and evaluate the extrapolation LP re-solving policy.
        ...
        for n_train_samples in self.train_sample_sizes:
            for sample_id in range(self.n_samples_per_size):
                ...
                initial_dual_variables[n_train_samples].append(
                    {supply_node_id: 0 for supply_node_id in self.graph.supply_nodes}
                )
        ...
        for n_train_samples in self.train_sample_sizes:
            for i in range(self.n_samples_per_size):
                ...
                log_output_line(self.instance_id, 'extrapolation_lp_resolving', n_train_samples, i,
                                rewards[n_train_samples][i], train_times[n_train_samples][i],
                                test_times[n_train_samples][i])
        ...
        for n_train_samples in self.train_sample_sizes:
            policy_output[n_train_samples] = PolicyOutput(
                rewards[n_train_samples], train_times[n_train_samples],
                test_times[n_train_samples], n_train_samples, 'extrapolation_lp_resolving'
            )
...
def main(demand_model):
    ...
    (optionally in parallel), and writes results to CSV.
    ...
    train_sample_sizes = [5**(i) for i in range(4)]
    ...
    for instance_id in range(num_instances):
        ...
        for n_train_samples in train_sample_sizes:
            for _ in range(n_samples_per_size):
                train_samples[instance_id][n_train_samples].append(
                    [train_generator.generate_sequence() for _ in range(n_train_samples)]
                )
    ...
    writer = OutputWriter(
        data_agnostic_policies, model_based_dynamic_programs,
        model_free_parametrized_policies, lp_resolving_policies,
        num_instances, train_sample_sizes, n_samples_per_size,
        include_optimal, results
    )

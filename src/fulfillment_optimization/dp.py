"""Dynamic programming solvers for optimal fulfillment policies.

Provides a unified DP framework with pluggable demand models (independent
or Markovian). Both variants share the same Bellman recursion logic,
differing only in how transition probabilities are computed.
"""
...
class _BaseDynamicProgram:
    """Shared DP logic for backward induction.

    Subclasses implement ``_transition_prob`` to define how demand
    ...
    def _compute_policy(self, inventory: Inventory, T: int, demand_params):
        """Core Bellman recursion shared by all DP variants.

        Args:
            inventory: Initial inventory levels.
            T: Time horizon.
            demand_params: Demand model parameters (passed to _transition_prob).

        Returns:
            DPOutput containing value_function and optimal_action.
        """
        inventory_states = self._generate_inventory_state_space(inventory, T)
        ...
        for inventory_state in inventory_states:
            for demand_node_id in self.graph.demand_nodes:
                value_function[inventory_state, T - 1, demand_node_id], optimal_action[inventory_state, T - 1, demand_node_id] = self._find_best_final_supply_node(demand_node_id, inventory_state)
            ...
            for inventory_state in inventory_states:
                for demand_node_id in self.graph.demand_nodes:
                    if sum(inventory_state) >= inventory.total_inventory - t:
                        value_function[inventory_state, t, demand_node_id], optimal_action[inventory_state, t, demand_node_id] = self._find_best_continuation(demand_node_id, inventory_state, value_function, demand_params, t)

        return DPOutput(value_function, optimal_action)

    def _find_best_final_supply_node(self, demand_node_id, inventory_state):
    ...
    def _find_best_continuation(self, demand_node_id, inventory_state, value_function, demand_params, t):
        ...
        for supply_node_id in demand_node.neighbors:
            if inventory_state[supply_node_id] > 0:
                next_inventory = list(inventory_state)
                next_inventory[supply_node_id] -= 1
                next_inventory = tuple(next_inventory)

                action_reward = 0
                for next_demand_node in self.graph.demand_nodes:
                    prob = self._transition_prob(demand_params, demand_node_id, next_demand_node, t)
                    action_reward += (self.graph.edges[supply_node_id, demand_node_id].reward + value_function[next_inventory, t + 1, next_demand_node]) * prob
        ...
        for next_demand_node in self.graph.demand_nodes:
            prob = self._transition_prob(demand_params, demand_node_id, next_demand_node, t)
            no_action_reward += value_function[inventory_state, t + 1, next_demand_node] * prob
    ...
    def _transition_prob(self, demand_params, current_demand_node_id, next_demand_node_id, t):
        """Return transition probability. Implemented by subclasses."""
        raise NotImplementedError

    def _generate_inventory_state_space(self, inventory: Inventory, T: int):
...
class IndependentDynamicProgram(_BaseDynamicProgram):
    ...
    def compute_optimal_policy(self, inventory: Inventory, T: int, p: Dict[int, List[float]]):
        ...
        """
        return self._compute_policy(inventory, T, p)

    def _transition_prob(self, p, current_demand_node_id, next_demand_node_id, t):
        return p[t + 1][next_demand_node_id]

    # Keep old public methods for backward compat
    def find_best_final_supply_node(self, demand_node_id, inventory_state):
        return self._find_best_final_supply_node(demand_node_id, inventory_state)

    def find_best_continuation(self, demand_node_id, inventory_state, value_function, p, t):
        return self._find_best_continuation(demand_node_id, inventory_state, value_function, p, t)

    def generate_inventory_state_space(self, inventory, T):
        return self._generate_inventory_state_space(inventory, T)


class MarkovianDynamicProgram(_BaseDynamicProgram):
    ...
    def compute_optimal_policy(self, inventory: Inventory, T: int, transition_matrix):
        ...
        Returns:
            DPOutput containing value_function and optimal_action mappings.
        """
        return self._compute_policy(inventory, T, transition_matrix)

    def _transition_prob(self, transition_matrix, current_demand_node_id, next_demand_node_id, t):
        return transition_matrix[current_demand_node_id, next_demand_node_id]
    ...
    def find_best_continuation(self, demand_node_id, inventory_state, value_function, transition_matrix, t):
        return self._find_best_continuation(demand_node_id, inventory_state, value_function, transition_matrix, t)

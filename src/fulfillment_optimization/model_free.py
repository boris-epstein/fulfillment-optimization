...
class ThresholdsFulfillment:
    ...
    def train(self, inventory: Inventory, train_samples: List[Sequence], optimizer_name: str = 'DE', budget: int = 1000, seed: int = 42):
        ...
        Args:
            inventory: Inventory for evaluation.
            train_samples: Training demand sequences.
            optimizer_name: Nevergrad optimizer name (default 'DE').
            budget: Number of optimizer evaluations.
            seed: Random seed.
...
class TimeSupplyEnhancedMPB(MultiPriceBalanceFulfillment):
    ...
    Adds per-supply-node bias terms (theta) and a time-dependent exponential
    decay factor (gamma) to the base multi-price balance opportunity cost.
...
class SupplyEnhancedMPB(MultiPriceBalanceFulfillment):
    ...
    Adds a constant offset theta_i to the opportunity cost for each supply node,
    trained via derivative-free optimization.
